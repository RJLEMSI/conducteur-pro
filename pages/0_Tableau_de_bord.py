...
STATUT_DISPLAY = {
    ...
    "envoyÃÂ©e": "Envoyee", "accepte": "Accepte", "refuse": "Refuse",
...
if ai_query and ai_query.strip():
    ...
    if any(kw in query_lower for kw in ["metre", "metrÃÂ©", "metr", "quantitatif"]):
        st.markdown("""
...
with planning_container:
    if gantt_data:
        ...
        fig_plan = px.timeline(
            df_gantt, x_start="Debut", x_end="Fin", y="Phase",
            color="Chantier",
            hover_data=["Chantier", "Statut", "Avancement"],
            color_discrete_map=chantier_color_map,
            title=""
        )
        fig_plan.update_yaxes(autorange="reversed")
        fig_plan.update_layout(height=max(350, len(gantt_data) * 40), margin=dict(l=0, r=0, t=10, b=0), showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=-0.3))
        ...
        for idx_ch, ch_nom in enumerate(unique_chantiers):
            ...
            label = f"\U0001f3d7\ufe0f {ch_nom} Ã¢ÂÂ {nb_phases} phase(s)" if nb_phases > 0 else f"\U0001f3d7\ufe0f {ch_nom} Ã¢ÂÂ Aucune phase"
            ...
            with st.expander(label, expanded=False):
                if phases_ch:
                    for ph in phases_ch:
                        ...
                        col_b.caption(f"{ph.get('date_debut', '?')} Ã¢ÂÂ {ph.get('date_fin', '?')}")
