...
Vue d'ensemble : chantiers actifs, planning global, finances, alertes.
"""
import sys, os
...
.kpi-value { font-size: 1.9rem; font-weight: 800; color: #0D3B6E; line-height: 1.1; }
...
with k1:
    st.markdown(f"""<div class="kpi-card">
        ...
        <div class="kpi-delta" style="color:#6B7280">{nb_planifies} planifié(s) · {nb_termines} terminé(s)</div>
    </div>""", unsafe_allow_html=True)

with k2:
    st.markdown(f"""<div class="kpi-card">
        <div class="kpi-value">{ca_total/1000:.0f}k€</div>
...
with k3:
    ...
    st.markdown(f"""<div class="kpi-card">
        <div class="kpi-value">{facture_tot/1000:.0f}k€</div>
...
with k4:
    ...
    st.markdown(f"""<div class="kpi-card">
        <div class="kpi-value">{encaisse_tot/1000:.0f}k€</div>
...
with k5:
    ...
    st.markdown(f"""<div class="kpi-card">
        <div class="kpi-value" style="color:{color_r}">{reste_fact/1000:.0f}k€</div>
...
with k6:
    ...
    icon_u  = "🔴" if nb_retard > 0 else ("🟡" if nb_urgent > 0 else "🟢")
    st.markdown(f"""<div class="kpi-card">
...
with col_left:
    ...
    if df_gantt.empty:
        ...
        all_starts = df_gantt["date_debut_dt"]
        all_ends   = df_gantt["date_fin_dt"]
        min_d = min(all_starts.min(), TODAY - timedelta(days=7))
        max_d = max(all_ends.max(), TODAY + timedelta(days=30))
        ...
        for _, row in df_gantt.sort_values("date_debut_dt").iterrows():
            ...
            av    = float(row.get("avancement_pct", 0))
            color = STATUS_COLOR.get(row.get("statut", ""), "#1B6CA8")
            badge_cls = {
                "En cours": "badge-actif", "Planifié": "badge-planifie",
                "Terminé": "badge-termine", "En retard": "badge-retard",
                "En attente": "badge-attente",
            }.get(row.get("statut", ""), "badge-planifie")
            ...
            <div style="margin-bottom:.7rem;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.25rem;gap:.5rem;">
                <span style="font-size:.8rem;font-weight:700;color:#0D3B6E;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"
                      title="{row['nom']}">{row['nom'][:42]}</span>
              ...
              <div style="background:#F1F5F9;border-radius:6px;height:18px;position:relative;overflow:visible;">
                ...
                <div style="position:absolute;left:{s_pct:.1f}%;width:{dpct:.1f}%;height:100%;
                            background:{color};opacity:.2;border-radius:6px;"></div>
                <!-- Bar progress (avancement) -->
                <div style="position:absolute;left:{s_pct:.1f}%;width:{dpct * av / 100:.1f}%;height:100%;
        ...
        st.markdown(f"""<p style="font-size:.72rem;color:#9CA3AF;margin-top:.3rem;">
            📍 Ligne rouge = aujourd'hui ({TODAY.strftime('%d/%m/%Y')})
...
with col_mid:
    ...
    if df_actifs.empty:
        ...
        for _, row in df_actifs.iterrows():
            ...
            p_enc    = (encaisse / budget * 100) if budget else 0
            reste    = budget - fact
            badge_cls = {
                "En cours": "badge-actif", "Planifié": "badge-planifie",
            ...
            <div class="ch-card">
              ...
              <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.3rem;margin-bottom:.45rem;">
                <div style="text-align:center;">
                  ...
                  <div style="font-size:.82rem;font-weight:700;color:#0D3B6E;">{budget:,.0f}€</div>
                </div>
                <div style="text-align:center;">
                  <div style="font-size:.65rem;color:#9CA3AF;">Facturé</div>
                  <div style="font-size:.82rem;font-weight:700;color:#1B6CA8;">{fact:,.0f}€</div>
                </div>
                <div style="text-align:center;">
                  <div style="font-size:.65rem;color:#9CA3AF;">Encaissé</div>
                  <div style="font-size:.82rem;font-weight:700;color:#059669;">{encaisse:,.0f}€</div>
                </div>
                <div style="text-align:center;">
                  <div style="font-size:.65rem;color:#9CA3AF;">Reste à fact.</div>
                  <div style="font-size:.82rem;font-weight:700;color:#D97706;">{reste:,.0f}€</div>
                </div>
              </div>
...
with col_right:
    ...
    if df_e.empty:
        ...
        df_e_sorted = df_e.sort_values("date_dt")
        for _, etape in df_e_sorted.iterrows():
            date_v = etape.get("date_dt")
            days_left = int((date_v - TODAY).days) if pd.notna(date_v) else 999

            if days_left < 0:
                bg = "#FEF2F2"; border = "#EF4444"
                tag = f"🔴 {abs(days_left)}j retard"
            elif days_left == 0:
                bg = "#FFF7ED"; border = "#F59E0B"
                tag = "🟡 Aujourd'hui"
            elif days_left <= 3:
                bg = "#FFF7ED"; border = "#F59E0B"
                tag = f"🟡 Dans {days_left}j"
            elif days_left <= 7:
            ...
            prio_c = {"Haute": "#DC2626", "Normale": "#6B7280", "Basse": "#9CA3AF"}.get(etape.get("priorite", ""), "#6B7280")
...
with st.expander("⚙️ Gérer les chantiers, étapes et exporter", expanded=False):
    ...
    with tab_exp:
        ...
        with c_ex2:
            ...
            imp_file = st.file_uploader("Importer JSON (export précédent)", type=["json"], key="imp_tdb")
            if imp_file:
                try:
