...
# Si dj connect, rediriger vers le tableau de bord
if is_authenticated():
...
with tab_register:
    with st.form("register_form", clear_on_submit=True):
        st.markdown("#### Crez votre compte ConducteurPro")
        ...
        reg_pwd = st.text_input("Mot de passe", type="password", placeholder="Minimum 8 caractres", key="reg_pwd")
        ...
        if submitted:
            if not reg_email or not reg_pwd:
                ...
                st.error("Le mot de passe doit contenir au moins 8 caractres.")
            elif reg_pwd != reg_pwd2:
                st.error("Les mots de passe ne correspondent pas.")
            else:
                with st.spinner("Cration du compte..."):
